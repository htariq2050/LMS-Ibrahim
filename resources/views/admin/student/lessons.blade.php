...
@section('title', $purchasedCourse->course->title ?? 'Default Course Title')
                ...
                <div>
                    ...
                    <div class="progress">
                        <div class="progress-bar" role="progressbar"
                            style="width: {{ min(intval($completionPercentage), 100) }}%;"
                            aria-valuenow="{{ min(intval($completionPercentage), 100) }}" aria-valuemin="0"
...
<div class="card">
    <div class="embed-responsive embed-responsive-16by9">
        ...
        @else
            <div class="text-center p-5">No video available.</div>
        @endif
    </div>
</div>
                ...
                <div class="col-md-4">
                    ...
                    <ul class="list-group list-group-fit" id="lesson-list">
                        @foreach ($purchasedCourse->course->lessons->sortBy('order') as $lesson)
                            ...
                            @endphp
                            <li class="list-group-item lesson-item {{ isset($currentLesson) && $currentLesson->id == $lesson->id ? 'active' : '' }}"
                                ...
                                <div class="media">
                                    ...
                                    <div class="media-body">
                                        <a href="{{ route('student.video', ['videoId' => optional($lesson->videos->first())->id]) }}" class="lesson-link text-dark">
                                            {{ $lesson->title }}
                                        </a>
                                    </div>
                            ...
                            @foreach ($relatedCourses as $related)
                                <div class="card card__course clear-shadow border">
                                    <div class=" d-flex justify-content-center">
                                        <a class="" href="#">
                                            <img src="{{ '/uploads/courses_cover_images/' . $related->cover_image }}"
                                                style="width:100%" alt="{{ $related->title }}">
                                        </a>
                                    </div>
                                    <div class="p-3">
                                        <div class="d-flex align-items-center">
                                            ...
                                            <div>
                                                <a class="text-body mb-1"
                                                    href="#"><strong>{{ $related->category->name }}</strong></a><br>
                                                <div class="d-flex align-items-center">
                                            ...
                                            <a href="{{ route('checkout', $related->id) }}"
                                                class="btn btn-primary ml-auto">${{ $related->price }}</a>
                                        </div>
                                    </div>
                                </div>
                            @endforeach
                    ...
                    success: function(response) {
                        if (response.success) {
                            ...
                            const videoUrl = response.video_url ?
                                `https://www.youtube-nocookie.com/embed/${response.video_url}?rel=0&modestbranding=1&showinfo=0` :
                                '';
    ...
    <script>
        ...
        function markLessonComplete() {
            // Make an AJAX request to mark the lesson as complete
            $.ajax({
                url: '{{ route('student.mark.lesson.complete') }}', // Adjust this route
                type: 'POST',
                data: {
                ...
                success: function(response) {
                    // Redirect to the next video or lesson
                    window.location.href = response.redirect_url;
                },
                error: function(xhr) {
                    console.error('Error marking lesson complete:', xhr);
                }
            });
        }
    </script>
